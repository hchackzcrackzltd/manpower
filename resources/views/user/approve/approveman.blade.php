...
@section('content')
  ...
  <div class="panel-body text-center">
    ...
    <form name="app" id="app" action="{{route('approveu.update',['type'=>$type_app,'app'=>$fm->id])}}" method="post">
      {{method_field('PATCH')}}
      {{csrf_field()}}
    </form>
    <form name="rej" id="rej" action="{{route('approveu.destroy',['type'=>$type_app,'app'=>$fm->id])}}" method="post">
      {{method_field('DELETE')}}
      {{csrf_field()}}
    </form>
  </div>

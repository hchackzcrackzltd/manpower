...
@section('content')
  ...
  @component('template.component.boxcontent')
    ...
    @slot('title')
      <i class="fa fa-pencil"></i> Form Request
    @endslot
    @slot('overlay',null)
  ...
  function validate3() {
    ...
    $('.sendf').on('click', function(event) {
      if (!validate()) {
        ...
        $('.addreq').attr('action', '{{route('manpowerreq.restore',['manpowerreq'=>$data->id])}}');
    ...
    $('.savefm').on('click', function(event) {
      if (!validate()) {
        ...
        $('.addreq').attr('action', '{{route('manpowerreq.resave',['manpowerreq'=>$data->id])}}');
        $('.addreq').submit();
      }
    });

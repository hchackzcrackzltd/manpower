        ...
        <div class="col-xs-12 col-md-3">
          <div class="form-group">
            ...
            <div class="input-group">
              ...
              <input type="text" class="form-control" id="code" value="{{$data->code}}" placeholder="Code" readonly>
            </div>
          </div>
        </div>
  ...
  <script>
    ...
    $('.send').on('click', function(event) {
      if ($(this).attr('data-act')==='save') {
        $('#fmrgn').attr('action', '{{route('resignreq.resave',['resignreq'=>$data->id])}}');
      }else {
        $('#fmrgn').attr('action', '{{route('resignreq.restore',['resignreq'=>$data->id])}}');
      }
